...
class FinanceFlow {
    ...
    initGoogleAuth() {
        ...
        if (clientID.includes("YOUR_GOOGLE_CLIENT_ID_HERE")) {
            ...
            if (btn) {
                btn.innerHTML = `
                    <button class="google-login-btn-custom" onclick="const id=prompt('Google Login Setup: Please enter your Google Client ID (from Google Cloud Console) to enable this feature:'); if(id){localStorage.setItem('google_client_id', id.trim()); location.reload();}">
                    ...
                    <div style="text-align: center; margin-top: 10px;">
                        <a href="#" style="font-size: 0.75rem; color: var(--text-muted); text-decoration: underline;" 
                           onclick="const id=prompt('Enter Google Client ID:'); if(id){localStorage.setItem('google_client_id', id.trim()); location.reload();}">
                            <i class="fas fa-cog"></i> Configure Google Client ID
                        </a>
                    </div>
                `;
            }
